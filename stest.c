...
#define START_UNIX_SOCK_PATH "/tmp/unix_start.server"
#define START2_UNIX_SOCK_PATH "/tmp/unix_start2.server"
...
void generate_ships(int ships[8][8]){
    for(int i = 0 ; i < 8 ; i++){
        for(int j = 0 ; j < 8 ;j++){
            if(rand()%100>80)ships[i][j]=1;
            else ships[i][j]=0;
        }
    }
}

void generate_shots(int shots[8][8]){
    for(int i = 0 ; i < 8 ; i++){
        for(int j = 0 ; j < 8 ;j++){
            shots[i][j]=0;
        }
         ...
         sval=send(msgsock,try,strlen(try),0);
            ...
            if(c == counter){
                ...
                if(strcmp(mes,"start")==0){
                    generate_ships(arr_info[c].myShips);
                    generate_shots(arr_info[c].myShots);
                    print_matrix(arr_info[c].myShips);

                    generate_ships(arr_info[c2].myShips);
                    ...
                    rc = send(client_sock, rec, strlen(rec), 0);
                    rc = send(client_sock, " ", 1, 0);
                    for(int i = 0 ; i < 8 ; i++){
                        for(int j = 0 ; j < 8 ; j++){
                            if(arr_info[c2].myShips[i][j]==0)rc = send(client_sock, "0" , 1 , 0);
                            else rc = send(client_sock, "1" , 1 , 0);
                            // printf("send %d ",rc);
                        }
                        for(int j = 0 ; j < 8 ; j++){
                            if(arr_info[c2].myShots[i][j]==0)rc = send(client_sock, "0" , 1 , 0);
                            else rc = send(client_sock, "1" , 1 , 0);
                            // printf("send %d ",rc);
                        }
                    }
   ...
   do {
      if ((msgsock = accept(sock,(struct sockaddr *) NULL,(socklen_t *) NULL)) == -1) 
        ...
        if((rd=read(fd, &buf_fifo, sizeof(buf_fifo))) > -1 && strcmp(buf_fifo,"/favicon.ico")){
            ...
            // if(counter>1){
            //     for(i = 0 ; i < counter ; i++){
            //         dprintf(1,"try to send lol\n");
            //         sval=send(arr[i],buf_fifo,strlen(buf_fifo),0);
            //         dprintf(1,"%d \n",sval);
            //     }
            // }         
      } 
